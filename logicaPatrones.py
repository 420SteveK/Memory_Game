...
class PatternMemoryLogic:
    ...
    def __init__(self):
        ...
        self.on_show_pattern_callback = None    # Called with cell coords to highlight (r,c)
        self.on_hide_pattern_callback = None    # Called with cell coords to unhighlight (r,c)
        self.on_info_update_callback = None     # Called with info string for UI label update
        self.on_fail_callback = None             # Called with failure message string
        self.on_success_callback = None          # Called with success message string and next level
        self.on_enable_input_callback = None     # Called when input phase starts, GUI can enable buttons
        self.on_disable_input_callback = None    # Called when input phase ends, GUI disables buttons
    ...
    def next_level(self):
        ...
        threading.Thread(target=self._show_pattern_sequence_thread, daemon=True).start()
    ...
    def _show_pattern_sequence_thread(self):
        ...
        threading.Thread(target=self._input_timeout_checker, daemon=True).start()
    ...
    def _input_timeout_checker(self):
        ...
        while self.input_enabled:
            ...
            if self.total_time_start and (now - self.total_time_start) > self.MAX_TOTAL_TIME:
                self._fail("Too slow! Total time exceeded 12 seconds for pattern input.")
                break
            time.sleep(0.1)
    ...
    def user_press(self, r, c):
        ...
        if len(self.user_sequence) == len(self.pattern):
            ...
            if total_elapsed > self.MAX_TOTAL_TIME:
                self._fail(f"Too slow! You took {total_elapsed:.1f}s, limit is {self.MAX_TOTAL_TIME}s.")
                return
            else:
                self._success()
